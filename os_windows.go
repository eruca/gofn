...
func get_appdata_folder_path() string {
	b := make([]uint16, syscall.MAX_PATH)
	ret, _, err := syscall.Syscall6(proc_sh_get_folder_path.Addr(), 5, 0,
		csidl_appdata, 0, 0, uintptr(unsafe.Pointer(&b[0])), 0)
	if int(ret) != 0 {
		panic(fmt.Sprintf("GetModuleFileNameW : err %d", int(err)))
	}
	return syscall.UTF16ToString(b)
...
func config_dir() string {
	return filepath.Join(get_appdata_folder_path(), "gofn")
}

func config_file() string {
	return filepath.Join(get_appdata_folder_path(), "gofn", "config.json")
}
